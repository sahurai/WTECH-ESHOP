...
CREATE TABLE book_categories (
    id uuid PRIMARY KEY DEFAULT uuid_generate_v4(),
    book_id INTEGER NOT NULL REFERENCES books(id) ON DELETE CASCADE,          -- Reference to book
    category_id INTEGER NOT NULL REFERENCES categories(id) ON DELETE CASCADE, -- Reference to category
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,                           -- Record update date
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,                           -- Record creation date
);
...
CREATE TABLE book_genres (
    id uuid PRIMARY KEY DEFAULT uuid_generate_v4(),
    book_id INTEGER NOT NULL REFERENCES books(id) ON DELETE CASCADE,     -- Reference to book
    genre_id INTEGER NOT NULL REFERENCES genres(id) ON DELETE CASCADE,   -- Reference to genre
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,                      -- Record update date
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,                      -- Record creation date
);
...
CREATE TABLE orders (
    id uuid PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,    -- User who placed the order
...
CREATE TABLE order_items (
    id uuid PRIMARY KEY DEFAULT uuid_generate_v4(),
    order_id INTEGER NOT NULL REFERENCES orders(id) ON DELETE CASCADE,    -- Reference to order
    book_id INTEGER NOT NULL REFERENCES books(id) ON DELETE CASCADE,      -- Reference to book
